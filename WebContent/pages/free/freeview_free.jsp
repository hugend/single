				...
				<div class="pagination">
					<ul>
						...
						<%if(pageNo==1){ %>
							&laquo; Previous
						<%}else{ %>
							<a href="/single/fr/list.do?category=free&page=<%=pageNo-1%>">&laquo; Previous</a>
						<%} %>
						</li>
							<%for(int i=firstPage;i<=lastPage;i++){ 
								...
								<%}else{ %>
								<li><a href="/single/fr/list.do?category=free&page=<%=i%>"><%=i %></a></li>
							<%}} %>
						<li class="next">
						<%if(pageNo==totalPage){ %>
							Next &raquo;
						<%}else{ %>
							<a href="/single/fr/list.do?category=free&page=<%=pageNo+1 %>">Next &raquo;</a>
						<%} %>
						</li>
					</ul>
				</div>
